...
First, plotting is your friend. Sometimes you can plot your design fine using `r pkg("ggplot2")`, but you often miss out on the inferential parts for the confidence intervals around estimates etc. So, it can be useful to use the `r pkg("sjPlot")` package which can take regression models and plot the model predictions. To decomponse an interaction, it is normally more useful to use the original / dummy-coded variables, as we are interested in the simple effects here. 
