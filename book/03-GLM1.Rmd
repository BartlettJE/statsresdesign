...
We will fit ordinal regression models using the `r pkg("ordinal")` [@Christensen2023] package as nicely supports cumulative links as regular regression models or scaled up to mixed effects models. 
...
- Rounding to two decimals, the slope coefficient converted to an odds ratio is `r fitb(answer = 4.66, num = TRUE, tol = .001)` [lower 95% CI = `r fitb(answer = 1.44, num = TRUE, tol = .001)`, upper 95% CI = `r fitb(answer = 16.31, num = TRUE, tol = .001)`].

- This suggests there are `r fitb(answer = 4.66, num = TRUE, tol = .001)` higher odds of a larger hire rating in the `r mcq(opts = c(x = "transcript", answer = "audio"))` condition compared to the `r mcq(opts = c(answer = "transcript", x = "audio"))` condition. 
