...
Participants then completed five free-text questions asking them about information in the study. Their responses were manually coded and received a 1 if they made a mistaken causal inference, or a 0 if they did not make (`DV1_1` to `DV1_5` in the data set). 
...
For the guided example, we will focus on `DV1_1` which coded `1` or `0` for whether participant's provided a mistaken causal inference in the question "Based on the news story, why is binge watching TV seen as bad?". So, we will test whether the probability of making a mistaken causal inference will be lower in the correction group compared to the no correction group.
...
Instead of counts, we can also express binary 0 and 1 data as a proportion or probability of making a mistake. If you take the mean of binary data, it tells you the probability, so we can calculate the proportion of participant's making a mistake in each group. 
...
So far, we have always used `r hl(lm())`, but R comes with another called `r hl(glm())` for generalised linear regression models. To demonstrate how we can set the link family, we can use it to recreate a regular linear regression model, but this time explicitly calling a normal distribution as the link (using its alternate name a Gaussian distribution) and `r hl("identity")` as the link function. 
...
An odds ratio is a ratio of the odds of the event happening in one group compared to the odds of the event happening in another group. In this context, a ratio of making a mistaken causal inference (1s compared to 0s) in the no correction group, compared to the correction group. 
...
Expressed this way, there are 1.65 [0.82, 3.38] lower odds of making a mistaken causal inference in the correction group compared to the no correction group. In this model, it is not statistically significant and we can see there is a decent amount of uncertainty around the estimate, with an odds ratio spanning above and below 1. 
...
This means the estimated probability of a mistaken causal inference in the correction condition is .35 or 35%. These values line up nicely with our original summary statistics, but keep in mind they are model estimates. They are closer here as its a very simple model and we are demonstrating the principles, but they will not line up this close when you have more complex models. 
...
- This suggests there are `r fitb(answer = 10.58, num = TRUE, tol = .001)` lower odds of making a mistaken causal inference in the `r mcq(opts = c(x = "no correction", answer = "audio"))` condition compared to the `r mcq(opts = c(answer = "no correction", x = "audio"))` condition. 
