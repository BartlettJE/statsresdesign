...
If you save the model as a .rds file, you can load them again using the `r hl(read_rds())` function from `r pkg("readr")` in the `r pkg("tidyverse")`. 
...
As a bonus extra, you can also use the `r pkg("emmeans")` package to calculate marginal effects on the posterior distribution. Its not important here as there is little we can learn from breaking down the interaction further, but it might come in handy in future. 
