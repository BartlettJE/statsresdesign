...
Our research question is: How are absorption (`r hl("Absorption_SUM")`) and mentalizing (`r hl("EQ_SUM")`) related to mystical experiences (`r hl("Mscale_SUM")`) as an outcome? Focus on entering the two variables as individual predictors at first, then explore an interaction. 
...
Finally, for sigma, we can use a relatively weak exponential prior with a change parameter of 1 since the estimate for the model SD was not reported in Coleman et al. We can plot all of these options below and piece them together with patchwork. 
...
The population level effects are relatively consistent with our priors. The intercept is slightly higher at 120 and both individual coefficients are positive predictors of mystical experiences. For the partial effects, we expect higher mystical experiences with both higher values of theory of mind and absorption. They are both clearly positive predictors with the 95% HDI no where near zero. 
...
plot(bayestestR::hdi(Coleman_fit))

```

Next, we can finally made sense of the interaction between theory of mind and absorption. 
...
plot(Coleman_conditional, 
     ...
     mean = T)[[1]] + 
  theme_classic() + 
  labs(y = "Mystical Experiences", x = "Empathizing Quotient")
```


Finally, lets look at the posterior predictive check.
