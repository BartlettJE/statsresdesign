...
- Looking at the guidelines from Wagenmakers et al. (2015), how could you describe the evidence in favour of the alternative hypothesis? `r mcq(opts = c(x = "Substantial", x = "Strong", x = "Very strong", answer = "Extreme))`

```{Zwaan explanation, webex.hide="Explain this answer"}
...
- Rounding to two decimals, what is the absolute (ignoring the sign) standardised mean difference (delta) in response time between congruent and incongruent trials? `r fitb(answer = 0.69, num = TRUE, tol = .001)`
...
For more information on ROPE within the `r pkg("bayestestR")` package, see [the online vignettes](https://easystats.github.io/bayestestR/articles/region_of_practical_equivalence.html). In the output, we have:
