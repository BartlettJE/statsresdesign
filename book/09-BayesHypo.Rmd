...
cauchy_dat <- data.frame(x = rcauchy(1e5, 0, 0.5),
                  y = rcauchy(1e5, 0, 0.707),
                  z = rcauchy(1e5, 0, 1))

cauchy_plot <- cauchy_dat %>% 
  ggplot() + 
  geom_density(aes(x = x), fill = "blue", alpha = alpha) + 
  geom_density(aes(x = y), fill = "red", alpha = alpha) +
  geom_density(aes(x = z), fill = "green", alpha = alpha) + 
  scale_x_continuous(limits = c(-5, 5)) + 
  theme_classic()

normal_dat <- data.frame(x = rnorm(1e5, 0, 0.5),
                  y = rnorm(1e5, 0, 0.707),
                  z = rnorm(1e5, 0, 1))
